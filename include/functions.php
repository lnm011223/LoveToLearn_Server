...
{   if($pvalue=='undefined') $pvalue="";
    if($pwhere!=-1&&!empty($pwhere)&&!empty($pvalue)){
        ...
        for ($i = 0; $i < count($fs); $i = $i + 1) {
            $fs[$i]=trim($fs[$i],'"');
            $pw1.=$aor.  $pfields. "=" . $pdelc . $fs[$i] . $pdelc ;
            $aor=" or ";
        }
        $pw1=empty($pw1) ?"" :" ( ".$pw1." ) ";
        $pw0.=((empty($pw0) || empty($pw1))  ? "" : " and ").$pw1 ;
    ...
    return array(
        ...
        'clientOptions' => array(
            ...
            'afterValidate' => 'js:function(form,data,hasError){
                if(hasError&&submitType.indexOf("_Nocheck")>=0){
                  hasError=!hasError;  
                }
                if(!hasError){
                    we.overlay("show");
...
function getf_url($flag=1){
    ...
    {
        return lcfirst(urldecode(get_url_explode()));
    }
}
...
function add_title_button($_thisController,$name,$id){
    $s='';
    if($name=='我的文章'){
        $s='<span class="back"><a class="btn" href="'.$_thisController->createUrl('blogText/Edit',array('id'=>$id)).'" onclick="we.back();"> <class="fa fa-reply"></i>'.
            '进入编辑'. '</a></span>';
    }
    if($name=='个人信息'){
        $s='<span class="back"><a class="btn" href="'.$_thisController->createUrl('user/admin',array('userId'=>$id)).'" onclick="we.back();"> <class="fa fa-reply"></i>'.
            '修改密码'. '</a></span>';
        $s.='<span class="back"><a class="btn" href="'.$_thisController->createUrl('user/UpdateInfo',array('userId'=>$id)).'" onclick="we.back();"> <class="fa fa-reply"></i>'.
            '修改信息'. '</a></span>';
...
function AES_encrypt($data,$password='s1c2n3u4csyyds!.#')
{
    $method = 'AES-128-ECB';//加密方法
    $options = 0;//数据格式选项（可选）
    $result = openssl_encrypt($data, $method, $password, $options);
    return $result;
}

function AES_decrypt($data,$password='s1c2n3u4csyyds!.#')
{
    $method = 'AES-128-ECB';//加密方法
    $options = 0;//数据格式选项（可选）
    $result = openssl_decrypt($data, $method, $password, $options);
    return $result;
}

function AES_decrypt1($data,$password)
{
    $method = 'AES-128-ECB';//加密方法
    $options = 0;//数据格式选项（可选）
    $result = openssl_decrypt($data, $method, $password, $options);
...
function RSA_decrypt($encrypted)  //RSA私钥解密
{
    //路径要改
    $privateKey = file_get_contents( BasePath::model()->getpngPath().'/static/admin/rsa_private_key.pem');
